document.addEventListener('DOMContentLoaded', function () {
  ...
  const addPhotoFigcaption = () => {
    document.querySelectorAll('#article-container images').forEach(item => {
      const altValue = item.title || item.alt
      if (!altValue) return
      const ele = document.createElement('div')
      ele.className = 'images-alt is-center'
   ...
   */
  const runLightbox = () => {
    btf.loadLightbox(document.querySelectorAll('#article-container images:not(.no-lightbox)'))
  }
  ...
  const runJustifiedGallery = (item, data, isButton = false, tabs) => {
    ...
    const completeFn = e => {
      ...
      btf.loadLightbox(item.querySelectorAll('images:not(.medium-zoom-image)'))
